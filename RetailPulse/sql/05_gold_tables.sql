...
-- =============================================================================
CREATE OR REPLACE TABLE `gcp-evening-batch-501811.retail_gold.daily_sales`
...
-- GOLD: monthly_sales
-- =============================================================================
CREATE OR REPLACE TABLE `gcp-evening-batch-501811.retail_gold.monthly_sales`
AS
WITH monthly_sales AS (
SELECT
    DATE_TRUNC(order_date, MONTH) AS sale_month,
    EXTRACT(YEAR FROM order_date) AS sale_year,
    EXTRACT(MONTH FROM order_date) AS sale_month_num,
...
SELECT
    *,
    LAG(gross_revenue) OVER (ORDER BY sale_month) AS prev_month_revenue,
    ROUND(
        SAFE_DIVIDE(
            gross_revenue - LAG(gross_revenue) OVER (ORDER BY sale_month),
            LAG(gross_revenue) OVER (ORDER BY sale_month)
        ) * 100,
        2
...
-- =============================================================================
CREATE OR REPLACE TABLE `gcp-evening-batch-501811.retail_gold.customer_lifetime_value`
...
FROM `gcp-evening-batch-501811.retail_silver.dim_customers` c
LEFT JOIN `gcp-evening-batch-501811.retail_silver.fact_orders` fo
...
-- =============================================================================
CREATE OR REPLACE TABLE `gcp-evening-batch-501811.retail_gold.repeat_customers`
AS
SELECT
  customer_id,
  total_orders,
...
FROM `gcp-evening-batch-501811.retail_gold.customer_lifetime_value`
WHERE total_orders > 0;

-- =============================================================================
-- GOLD: top_products
-- =============================================================================
CREATE OR REPLACE TABLE `gcp-evening-batch-501811.retail_gold.top_products`
AS
SELECT
  foi.product_id,
  p.product_name,
...
-- GOLD: top_categories
-- =============================================================================
CREATE OR REPLACE TABLE `gcp-evening-batch-501811.retail_gold.top_categories`
AS
SELECT
  foi.category,
  SUM(foi.line_total) AS total_revenue,
...
-- =============================================================================
CREATE OR REPLACE TABLE `gcp-evening-batch-501811.retail_gold.state_wise_sales`
AS
SELECT
...
-- =============================================================================
CREATE OR REPLACE TABLE `gcp-evening-batch-501811.retail_gold.brand_performance`
AS
SELECT
  foi.brand,
  foi.category,
...
-- =============================================================================
CREATE OR REPLACE TABLE `gcp-evening-batch-501811.retail_gold.payment_analysis`
AS
SELECT
  dp.payment_method,
  dp.payment_status,
...
FROM `gcp-evening-batch-501811.retail_silver.dim_payments` dp
LEFT JOIN `gcp-evening-batch-501811.retail_silver.fact_orders` fo
...
-- =============================================================================
CREATE OR REPLACE TABLE `gcp-evening-batch-501811.retail_gold.average_order_value`
AS
SELECT
  'ALL' AS dimension,
...
-- =============================================================================
CREATE OR REPLACE TABLE `gcp-evening-batch-501811.retail_gold.executive_kpis`
AS
SELECT
  (SELECT COUNT(DISTINCT customer_id) FROM `gcp-evening-batch-501811.retail_silver.dim_customers`) AS total_customers,
  (SELECT COUNT(DISTINCT order_id) FROM `gcp-evening-batch-501811.retail_silver.fact_orders` WHERE is_revenue_eligible) AS total_orders,
  (SELECT ROUND(SUM(total_amount), 2) FROM `gcp-evening-batch-501811.retail_silver.fact_orders` WHERE is_revenue_eligible) AS total_revenue,
  (SELECT ROUND(AVG(total_amount), 2) FROM `gcp-evening-batch-501811.retail_silver.fact_orders` WHERE is_revenue_eligible) AS avg_order_value,
  (SELECT COUNT(*) FROM `gcp-evening-batch-501811.retail_gold.repeat_customers` WHERE is_repeat_customer) AS repeat_customers,
  (SELECT ROUND(SAFE_DIVIDE(
    (SELECT COUNT(*) FROM `gcp-evening-batch-501811.retail_gold.repeat_customers` WHERE is_repeat_customer),
    (SELECT COUNT(*) FROM `gcp-evening-batch-501811.retail_gold.repeat_customers`)
